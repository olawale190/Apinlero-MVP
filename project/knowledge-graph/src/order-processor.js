/**
 * Àpínlẹ̀rọ Knowledge Graph - Order Processor
 *
 ...
 *   E7:  "🍚 2" (emoji + number) → preprocessed in intent-classifier
...
function isConfirmation(text) {
  const lower = text.toLowerCase().trim();
  return /^(yes|yeah|yep|yea|confirm|ok|okay|sure|go\s*ahead|proceed|👍|✅)$/i.test(lower);
}

/** Check if message is a rejection / change request */
function isRejection(text) {
  const lower = text.toLowerCase().trim();
  return /^(no|nope|nah|cancel|change|wait|hold\s*on|stop|❌)$/i.test(lower);
}
